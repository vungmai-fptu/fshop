    ...
    <div class="row pageBtn">
        <div class="col" style="text-align: right;">
            ...
            <form action="<c:url value="/home/search.do" />">
                <button type="submit" class="btn btn-danger btn-sm btn-info" name="op" value="FirstPage" title="First Page" <c:if test="${pageSearch==1}">disabled</c:if>><i class="bi bi-chevron-bar-left"></i></button>
                <button type="submit" class="btn btn-danger btn-sm btn-info" name="op" value="PreviousPage" title="Previous Page" <c:if test="${pageSearch==1}">disabled</c:if>><i class="bi bi-chevron-left"></i></button>
                <button type="submit" class="btn btn-danger btn-sm btn-info" name="op" value="NextPage" title="Next Page" <c:if test="${pageSearch==totalPageSearch}">disabled</c:if>><i class="bi bi-chevron-right"></i></button>
                <button type="submit" class="btn btn-danger btn-sm btn-info" name="op" value="LastPage" title="Last Page" <c:if test="${pageSearch==totalPageSearch}">disabled</c:if>><i class="bi bi-chevron-bar-right"></i></button>
                <input type="text" name="gotoPage" value="${pageSearch}" class="btn btn-danger btn-sm btn-outline-info" style="text-align: right;color:#fff;width: 32px;" title="Enter page number"/>
...
<c:if test="${empty list}">
    <div class="search-empty">
        <img src="${root}/img/search-empty.png" class="search-empty-icon"/>
        <div class="search-empty-title">Không tìm thấy kết quả nào</div>
        <div class="search-empty-hint">Hãy thử sử dụng các từ khóa chung hơn</div>
    </div>
</c:if>
